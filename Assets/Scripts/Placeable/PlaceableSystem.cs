...
public class PlaceableSystem : MonoBehaviour
{
    ...
    {
        tileApp = main_tilemap.GetTile(main_tilemap.WorldToCell(previousCellLocation));
    }
    ...
    {
        #region PreviewTile
        main_tilemap.SetTile(main_tilemap.WorldToCell(previousCellLocation), tileApp);
        ...
        else if (InputSystem.current.ReadInteractButtonUp() && selectedObj != null)
        {
            ...
            if (InputSystem.current.ReadMouse(groundMask) != Vector3.zero)
            {
                ...
                if (colonna != null)
                {
                    Vector3 posInColumn = new Vector3(0, colonna.AddToColumn(placeableScript.torretta) - 0.75f, 0);
